...
# Optional: crop region for compact visualizations in vignette
xobj <- xobj[, xobj$x < 5000 & xobj$y < 5400]
...
For this tutorial, we use the preprocessed Seurat object directly:

```{r}
xobj <- readRDS(url("https://alserglab.wustl.edu/files/fgsea/xenium-prime-ffpe-human-ovarian-cancer.rds"))
xobj <- ScaleData(xobj, verbose = FALSE)
```
...
p1 <- ImageDimPlot(xobj, group.by = "annotation", 
                   dark.background = FALSE, size = 0.2, 
                   flip_xy = TRUE) + 
    theme(legend.position = "none")

p1 <- suppressMessages(p1 + coord_flip() + scale_x_reverse() + theme(aspect.ratio = 1.0))

p2 <- DimPlot(xobj, group.by = "annotation", raster = TRUE) + coord_fixed()
...
gsets <- msigdbr("human", collection = "H")
